...
const ReportForm = ({ onSubmit, onCancel }) => {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.name) {
      newErrors.name = "Report name must be at least 3 characters long";
    }
    if (!formData.date) {
    ...
    <Modal onClose={onCancel}>
      <div className="form-container">
        <div className="form-header">
          ...
          <p className="form-subtitle">Enter the report details below</p>
        </div>

        <form onSubmit={handleSubmit} className={styles.form}>
          <div className={styles.formGroup}>
            <label className="form-label">
                Report Name <span className={styles.mandatory}>*</span>
            </label>
            <input
              type="text"
              name="name"
